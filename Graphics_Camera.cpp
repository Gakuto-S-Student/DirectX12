...
void GraphicsCamera::Set3D()
{
	...
	XMMATRIX proj = XMMatrixPerspectiveFovLH(XM_PIDIV2, 1280 / 780, 0.1f, 100.0f);
	...
	*viewBuffer = view;
	...
	XMMATRIX* projBuffer;
	m_projectionBuffer->Map(0, nullptr, (void**)&projBuffer);
	*projBuffer = proj;
	m_projectionBuffer->Unmap(0, nullptr);

	Graphics::Get()->Context()->SetGraphicsRootConstantBufferView(1, m_viewBuffer->GetGPUVirtualAddress());			// view
	Graphics::Get()->Context()->SetGraphicsRootConstantBufferView(2, m_projectionBuffer->GetGPUVirtualAddress());	// projection
}
